...
class TelegramAutoSenderTester:
    ...
    def test_subscription_system(self):
            ...
            try:
                ...
                if plans_response.status_code == 200:
                    ...
                    if plans:
                        ...
                        if premium_plan:
                            headers = {'Authorization': f'Bearer {self.access_token}'}
                            upgrade_data = {'plan_id': premium_plan['id']}
                            
                            # Note: We're passing plan_id as JSON body, but the endpoint expects it as a path parameter
                            # Let's test the correct format
                            response = self.session.post(f"{BASE_URL}/subscription/upgrade", 
                                                       json=upgrade_data, headers=headers)
                            
                            if response.status_code == 422:
                                # Try with plan_id as query parameter
        ...
        try:
            response = self.session.post(f"{BASE_URL}/subscription/upgrade", json={'plan_id': 'test_plan'})
